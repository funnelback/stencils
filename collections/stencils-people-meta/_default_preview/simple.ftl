...
<html lang="en-us">
  ...
  <body class="container<#if layoutType?? && layoutType != ''>-${layoutType}</#if>"<#if question.collection.configuration.valueAsBoolean("ui.modern.session")> data-ng-app="Funnelback" data-ng-controller="DefaultCtrl" </#if>>
    <#--<@fb.ViewModeBanner /> -->
    
    <@s.InitialFormOnly>
        <@people_controller.InitialForm image="${largeLogoImg}"/>
    </@s.InitialFormOnly>
    ...
    <@s.AfterSearchOnly>
        ...
        <section id="search-main" class="row" data-ng-show="isDisplayed('results')">
           <@people_view.BreadCrumb />
           <@people_controller.Facets names=["Department", "Expertise"]/>
          <div id="search-content" class="col-md-<@s.FacetedSearch>9</@s.FacetedSearch><@s.FacetedSearch negate=true>12</@s.FacetedSearch>">
            ...
            <@people_controller.azList rmcField="C" />
            <@core_controller.QueryHistory />
            <@core_controller.Scopes />
            <@people_controller.Count />
